...
class ExpectedReturnSecondScene(Scene):
    def construct(self):
        ...
        keep_equation_object = VGroup(right_term)
        ...
        self.play(
            *[
                ...
                if mob not in [right_term, keep_equation_object, title]
            ],
            right_term.animate.set_color(WHITE).shift(UP * 4 + LEFT * 0.5),
        )

        # Add description
        self.wait(2)
        intuitive_text = Text(
            "This expression represents the expected return (value function) from the\n"
            "next state s' onward weighted by the probability that each s' actually occurs.",
            font_size=27,
            t2c={
                "probability that each s' actually occurs.": BLUE,
                "expected return (value function) from the\n" "next state s'": GREEN,
            },
        ).next_to(keep_equation_object, DOWN, buff=1)

        self.play(
            Write(intuitive_text),
            right_term[0].animate.set_color(BLUE),
            right_term[1].animate.set_color(GREEN),
        )
