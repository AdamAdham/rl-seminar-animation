...
class ExpectedReturnFirstScene(Scene):
    def construct(self):

        # Motivation text
        title = Text("Expected Return", font_size=50).to_edge(UP)
        ...
        rearranged_equation = MathTex(
            r"=",
            r"\sum_{a}\pi(a\mid s)",
            r"\sum_{r}r\sum_{s'}p(r,s'\mid s,a)",
        ).next_to(left_term, DOWN, buff=0.5)

        self.wait(3)

        self.play(Create(rearranged_equation))

        self.wait(2)

        expected_reward_function = (
            MathTex(
                r"\text{Using Expected Reward: }",
                r"\mathbb{E}\!\left[ R_{t+1} \mid s,\; a \right]",
                r"=",
                r"\sum_{r} r \sum_{s'}\, p(s', r \mid s,a)",
            )
            .next_to(rearranged_equation, DOWN, buff=0.5)
            .scale(0.8)
        )

        self.play(Create(expected_reward_function))

        self.wait(2)

        self.play(
            expected_reward_function[-1].animate.set_color(BLUE),
            rearranged_equation[-1].animate.set_color(BLUE),
        )
        ...
        final_left_term = MathTex(
            ...
            r"\sum_{a}\pi(a\mid s)",
            r"\mathbb{E}\!\left[ R_{t+1} \mid s,\; a \right]",
        ).next_to(expected_reward_function, DOWN, buff=0.5)

        final_left_term[-1].set_color(BLUE)

        self.play(Create(final_left_term))

        # Remove all except last equation to provide a description
        self.wait(2)
        self.play(
            *[
                FadeOut(mob)
                for mob in self.mobjects
                if mob not in [final_left_term, title]
            ],
            final_left_term.animate.move_to([0, final_left_term.get_y(), 0]).shift(
                UP * 4.2
            ),
        )
        ...
        self.wait(2)
        intuitive_text = Text(
            "This expression gives the expected immediate reward, where each \npossible action a"
            "is weighted by the probability that our policy selects it.\n"
            "Intuitively, it tells us what reward we can expect on average if \n we follow the policy "
            "from state s.",
            font_size=28,
            t2c={
                "expected immediate reward": BLUE,
                "weighted by the probability that our policy": GREEN,
            },
        ).next_to(final_left_term, DOWN, buff=1)

        self.play(Write(intuitive_text), final_left_term[2].animate.set_color(GREEN))
